...
public class SwiftKryptografiPlugin: NSObject, FlutterPlugin {
	public static func register(with registrar: FlutterPluginRegistrar) {
		let channel = FlutterMethodChannel(name: "kryptografi", binaryMessenger: registrar.messenger())
		let instance = SwiftKryptografiPlugin()
	...
	public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
		...
		switch (call.method) {
			case "encryptBase64":
    			...
    			let res = encryptDataBase64(plainText: text, publicKey: publicKey)
    			result(res)
			case "decrypt":
				let text : String = argsMap["txt"] as! String
            ...
            case "decryptWithPublicKey":
                let plainText : String = argsMap["plainText"] as! String
                let publicKey : String = argsMap["publicKey"] as! String
                let res = decryptRsaBase64Encrypted(plainText, withPublicKeyBase64: publicKey)
                print(res)
                result(res)
            case "encryptUTF8":
				let text : String = argsMap["txt"] as! String
    			let publicKey : String = argsMap["publicKey"] as! String
    			let res = encryptDataUTF8(plainText: text, publicKey: publicKey)
    			result(res)
    ...
    private func encryptDataBase64(plainText: String, publicKey: String) -> String {


        let publicKey = try! PublicKey(pemEncoded: publicKey)
        let clear =  ClearMessage(data: Data(base64Encoded: plainText, options: .ignoreUnknownCharacters)!)
		let encrypted = try! clear.encrypted(with: publicKey, padding: .PKCS1)
		return encrypted.base64String
    }

        private func encryptDataUTF8(plainText: String, publicKey: String) -> String {
            let publicKey = try! PublicKey(pemEncoded: publicKey)
    ...
    func decryptRsaBase64Encrypted(_ string: String, withPublicKeyBase64: String) -> String? {
        print(string)
       print(withPublicKeyBase64)
        if let encrypted = Data(base64Encoded: string, options: Data.Base64DecodingOptions.init(rawValue: 0)) {
             let data = RSAUtils.decryptWithRSAPublicKey(encrypted, pubkeyBase64: withPublicKeyBase64, keychainTag: "")
